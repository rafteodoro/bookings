...
import (
	...
	"github.com/rafteodoro/bookings/internal/driver"
	"github.com/rafteodoro/bookings/internal/forms"
	...
	"github.com/rafteodoro/bookings/internal/repository"
	"github.com/rafteodoro/bookings/internal/repository/dbrepo"
)
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// assume that is no month or year specified
	now := time.Now()
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// Get the block map from the session. Loop through entire map, and if we have an entry in the map that 
		// does not exist in our posted data, and if the restriction ID > 0, then it's a block that needs removing
